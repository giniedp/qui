...
import { ControlViewModel, getModelValue, registerComponent, renderControl, ValueSource } from './core'
import { call, isArray } from './utils'

/**
 * @public
 */
export type Vector = number[] | { [key: string]: number } | { [key: number]: number }
  ...
  return {
    view: () => {
      return renderControl(node, (data) => {
        ...
        return !visible ? null : keys.map((field) => {
